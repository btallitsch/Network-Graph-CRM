...
export const getUserNodes = async (userId: string): Promise<NodeData[]> => {
  const q = query(nodesCol, where('userId', '==', userId), orderBy('createdAt', 'desc'))
  const snap = await getDocs(q)
  return snap.docs.map((d) => ({ id: d.id, ...d.data() } as NodeData))
}
...
export const subscribeToNodes = (
  ...
  return onSnapshot(q, (snap) => {
    const nodes = snap.docs.map((d) => ({ id: d.id, ...d.data() } as NodeData))
    callback(nodes)
  })
}
...
export const subscribeToEdges = (
  ...
  const q = query(edgesCol, where('userId', '==', userId), orderBy('createdAt', 'desc'))
  return onSnapshot(q, (snap) => {
    const edges = snap.docs.map((d) => ({ id: d.id, ...d.data() } as EdgeData))
    callback(edges)
  })
}
